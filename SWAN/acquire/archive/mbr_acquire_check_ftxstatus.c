...
int main(int argc, char *argv[])
{
    ...
    char *pcDevice = NULL;
    struct pcap_pkthdr *pstPacketHeader = NULL;
    u_char *pbPacket = NULL;
    ...
    while (1)
    {
        iRet = pcap_next_ex(g_pstHandle, &pstPacketHeader, &pbPacket);
        if (-1 == iRet)
        {
