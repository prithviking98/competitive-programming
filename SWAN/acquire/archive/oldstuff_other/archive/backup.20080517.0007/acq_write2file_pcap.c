...
#define DAS_CHK_SIZE_DATABLOCK      (1056 * 64)
#define DAS_CHK_SIZE_DATABLOCK_DT   (DAS_CHK_SIZE_DATABLOCK / sizeof(DAS_SHM_DATATYPE))

#define DAS_PKTSPERBLOCK            (DAS_CHK_SIZE_DATABLOCK / 1056)
...
int main(int argc, char *argv[])
{
    ...
    while (DAS_TRUE)
    {
        ...
        if (iBufLeft <= DAS_CHK_SIZE_DATABLOCK)
        {
            ...
            /* wait for the writer to start filling this buffer with fresh data */
            while (stBlockInAdds[iBuffer].pstBlock->iFlagData != DAS_STATUS_FRESH)
            {
