...
// keep it equal to SIZE_FULL_PACKET
...
//    struct packet_mreq stPacketMReq = {0};
    int iSocket = 0;
    int i = 0;

    /* SOCK_RAW gives the whole packet, including the Ethernet header */
    iSocket = socket(PF_PACKET, SOCK_RAW, htons(ETH_P_ALL));
    /* SOCK_DGRAM gives the packet starting from the IP header */
...
//    stPacketMReq
//    setsockopt(iSockeet, SOL_PACKET, PACKET_ADD_MEMBERSHIP, 
...
#if 0
    stSockAddr.sin_family = AF_INET;          /* host byte order */
    stSockAddr.sin_port = htons(PORT);          /* short, network byte order */
    stSockAddr.sin_addr.s_addr = INADDR_ANY;    /* automatically fill with my IP */
    (void) memset(stSockAddr.sin_zero, '\0', sizeof(stSockAddr.sin_zero));

    iRet = bind(iSocket, (struct sockaddr *) &stSockAddr, sizeof(stSockAddr));
    if (-1 == iRet)
    {
        perror("bind");
        return -1;
