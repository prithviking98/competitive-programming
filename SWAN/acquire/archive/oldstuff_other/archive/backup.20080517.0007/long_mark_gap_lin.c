...
/*   chk_n_rmmark_lin.c   major modification from chk_n_rmmark_ws.c
  ...
  ------------------------------------------------------- */
/* similar to mark_gap_lin.c, but now allows for markere that are repeated versions
  4-nibble codes .... desh 1-May-2008  */
...
/* void main(int argc, char *argv[]) */
	...
	if(argc<nargs)
	  {printf("usage....\n");
	   printf("long_mark_gap <infile> <n_repeat_FFFF> <counter_val_byte_distance> <nslip_blocks_allowed>\n");
       if(argc>1)
	  ...
	  {sscanf(argv[1],"%su",in_path);
	   sscanf(argv[2],"%du",&nrepeat);
	   sscanf(argv[3],"%du",&ndist_bytes);
...
/************************************************************/
       ...
       unsigned int irepeat_found = 0;
       unsigned int yet_to_pick_packet_count = 0;
       unsigned int block_no_to_wait_for;
       long int expected_l_of_counter; 
       long int expected_l_of_counter_in_next_block; 
       unsigned int remembered_last_mark;
...
// jayanth
        ...
        unsigned int jgap = 0;
        unsigned int jnumoccurffff = 0;
        unsigned int jslips = 0;
...
/* read the header bytes (2048) if has_header == 1 */
       ...
       else        /* no header */
	  {
	   ...
	   scanf("%d",&ofset_wrd);
#endif
        ofset_wrd = (512*2 + nrepeat*2 + ndist_bytes + 4)/2;    /* 515; for packet of 1030 bytes */
#if 0
...
//TEST_BLOCK            
    for(l=0;l<blk_size;l++)
	{
       ...
       /* see if there is a pending pick up for packet count value */
       if(yet_to_pick_packet_count == 1)
       { if(block_no_to_wait_for == i)
         {
          jcuraddr = (unsigned char *) &a[expected_l_of_counter_in_next_block]; 
          mark = (unsigned int) ntohl((*(unsigned int *) (jcuraddr)));
          //printf("c = %x, m = %x, i = %d, gap = %d\n", jcounterval, mark, i, remembered_last_mark);
          fflush(stdout);
          if (mark != (jcounterval + 1))
                {
                    //fprintf(stderr, "slip!\n");
                    printf("slipped_after_wait!");
                    printf("c = %x, m = %x, i = %d, lm = %d slip_size= %d\n", jcounterval, mark, i, remembered_last_mark,mark-jcounterval);
                    //printf("c = %x, m = %x, i = %d, lm = %d\n", jcounterval, mark, i, remembered_last_mark);
                    fflush(stdout);
                    //printf (" block no, mark,gap    : %ld %x %d %d\n",i,mark,remembered_last_mark,ofset_wrd); 
                //    missed_cnt++;
                    ++jslips;
                    //getchar();
                }
		...
		if(irepeat_found == nrepeat)
        {

            ++jnumoccurffff;
            //printf("no. of occurrences of %d ffff: %d, points after last %d ffff = %d\n", nrepeat, jnumoccurffff, nrepeat, jgap);
            jgap = 0;
          if(first_time < 1)
          {
            if(last_mark >= ofset_wrd) 
		    {
              ...
              mark = a[l];
#endif
              expected_l_of_counter = l + (ndist_bytes+2)/2;
              if((expected_l_of_counter+1) < blk_size)   /* we can hope to pick the counter value in the already read block */
              {
                jcuraddr = (unsigned char *) &a[l]; 
                mark = (unsigned int) ntohl((*(unsigned int *) (jcuraddr+ndist_bytes+2)));
                ...
                if (mark != (jcounterval + 1))
                {
                    //fprintf(stderr, "slip!\n");
                    //printf("slipped!");
                    //printf("c = %x, m = %x, i = %d, lm = %d slip_size= %d\n", jcounterval, mark, i, last_mark,mark-jcounterval);
                    printf("Slip detected! ");
                    printf("Previous = %X, current = %X, diff = %d\n", jcounterval, mark, (mark - jcounterval));
                    fflush(stdout);
        ...
        /* <-- Modified by Jayanth Chennamangalam */
              ...
              else   /* we need to wait for the next read cycle to pick the counter value */
              {
                expected_l_of_counter_in_next_block = expected_l_of_counter - blk_size;
                remembered_last_mark = last_mark;
                block_no_to_wait_for = i + 1;
                yet_to_pick_packet_count = 1;
              }
          ...
          else
          {
              ...
              if((expected_l_of_counter+1) < blk_size)   /* we can hope to pick the counter value in the already read block */
              {
                ...
                mark = (unsigned int) ntohl((*(unsigned int *) (jcuraddr+ndist_bytes+2)));
                jcounterval = mark;
                printf("starting  m = %x, i = %d, lm = %d\n", mark, i, last_mark- nrepeat);
                fflush(stdout);
                last_mark = 0;
                first_time = 0;
              }
...
okay:     mark_locn[n_mark] = blk_size;
	...
	{j = i_out - out_due_set*out_size;
	 ...
	 if(j>0)
      ...
      printf(" no. of slip events (miss) : %d\n",missed_cnt);
      printf(" no. of slip events (slip): %d\n",jslips);
      printf(" no. of markers found : %ld\n",total_markers);
