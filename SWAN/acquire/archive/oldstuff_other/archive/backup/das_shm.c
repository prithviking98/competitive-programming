...
//TODO: do away with the header update - add a new field for seglen
/*
...
int DAS_UpdateShmHeader(DAS_BYTE *pbNewHeaderInfo, int iPID)
{
    ...
    //TODO: remove void typecasting from header, once its type is changed to void*
...
//TODO: incorporate iShmInfoID and szKeyFile
#if 0
void DAS_HandleSIGINT(int iSigNo)
{
    ...
    //TODO: fix this
    int iPID = 0;   /* for compilation */

    /* locate shminfo */
    pstShmInfo = (DAS_SHMINFO_S *) DAS_ShmAlloc(DAS_KEY_FL_OUT,
                                                DAS_ID_SHMINF0,
                                                sizeof(DAS_SHMINFO_S),
                                                0444, /* r--r--r-- */
                                                &piShmIDShmInfo,
    ...
    if (NULL == pstShmInfo)
    {
        printf("DAS_HandleSIGINT(): ERROR in DAS_ShmAlloc() for pstShmInfo\n");
        return;
    }
    ...
    pstShmHeader = DAS_GetHeader(iPID);
    if (NULL == pstShmHeader)
    {
        printf("DAS_HandleSIGINT(): ERROR in DAS_GetHeader()\n");
        return;
    }
    ...
    for (iBlockID = pstShmInfo->iFirstBlockID;
         ...
         ++iBlockID)
    {
        iRet = DAS_DeleteBlock(iBlockID, DAS_KEY_FL_OUT, pstShmInfo, iPID);
        if (iRet != DAS_RET_SUCCESS)
        {
            printf("DAS_HandleSIGINT(): ");
            printf("ERROR in DAS_DeleteBlock() for block %d\n", iBlockID);
            assert(DAS_RET_SUCCESS == iRet);   /* abnormal behaviour */
            exit(1);
        }
    }
