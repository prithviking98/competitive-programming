...
#define LIMIT_SLEEP         (2 * SIZE_PACKET)
/* sleep time in microseconds */
#define SLEEPTIME_MORE      30
#define SLEEPTIME_LESS      10
...
int main(int argc, char *argv[])
{
    ...
    while (DAS_TRUE)
    {
        ...
        if ((pstShmInfoIn->iWrittenData - iReadData) > LIMIT_SLEEP)
        {
            (void) usleep(SLEEPTIME_MORE);
        }
        else
        {
            (void) usleep(SLEEPTIME_LESS);
        }
